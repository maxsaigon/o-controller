...
test('Windows resolves npm without using a shell', () => {
  assert.equal(resolveStageCommand('npm', 'win32'), 'npm.cmd');
  assert.equal(resolveStageCommand('node', 'win32'), 'node');
  assert.equal(resolveStageCommand('npm', 'linux'), 'npm');
